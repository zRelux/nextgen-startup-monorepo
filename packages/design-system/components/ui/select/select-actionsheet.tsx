...
const ActionsheetItemText = React.forwardRef(
	(
		{ isTruncated, bold, underline, strikeThrough, size, className, ...props }: IActionsheetItemTextProps,
		ref?: any,
	) => {
		return (
...
const ActionsheetIcon = React.forwardRef(
	({ className, as: AsComp, size = "sm", ...props }: IActionsheetIconProps & { as?: any }, ref?: any) => {
		if (AsComp) {
			return (
				<AsComp
