...
type ILinkProps = React.ComponentProps<typeof UILink> & VariantProps<typeof linkStyle>;
const Link = React.forwardRef(({ className, ...props }: { className?: string } & ILinkProps, ref?: any) => {
	return <UILink ref={ref} {...props} className={linkStyle({ class: className })} />;
});

type ILinkTextProps = React.ComponentProps<typeof UILink.Text> & VariantProps<typeof linkTextStyle>;
const LinkText = React.forwardRef(
	({ className, size = "md", ...props }: { className?: string } & ILinkTextProps, ref?: any) => {
		return (
			<UILink.Text
				ref={ref}
				{...props}
